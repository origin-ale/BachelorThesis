...
Lo spazio delle fasi del sistema è a rigore $T^*Q = T^*\big(\R^3 \times SO(3)\big)$. Esso è tuttavia isomorfo a $T^*\R^3 \times T^*SO(3)$; si userà quindi come spazio delle fasi $X =T^*\R^3 \times T^*SO(3)$.
...
\begin{equation*}
\Phi^{\vec{a}}_{\R^3}: \vec{q} \mapsto  \vec{a} + \vec{q}
\end{equation*}
dove il $+$ denota l'operazione di addizione definita in $\R^3$ inteso come spazio vettoriale. Si definisca la sua azione su $X$ come 
\begin{equation*}
  \Phi^{\vec{a}}_{\R^3}: (\vec{a} + \vec{q},\rho, \vec{\dot{q}}, \dot{\rho}^*) \longmapsto (\vec{q},\rho, \vec{\dot{q}}^*, \dot{\rho}^*)
\end{equation*}
dove $\vec{q} \in \R^3, \rho \in SO(3), \vec{\dot{q}}^* \in T^*_{\vec{q}}\R^3$ e $\dot{\rho}^* \in T^*_{\rho}SO(3)$. Questa azione è ben definita su $Q$ e genera quindi un'azione hamiltoniana su $X$ per il teorema di Noether.\\[11pt]
