...
Si possono ora rivisitare le formulazioni lagrangiana ed hamiltoniana esposte nel capitolo 1, avvalendosi degli strumenti sviluppati nel capitolo 2, per chiarire la natura dello spazio delle configurazioni, di quello delle fasi, e il legame tra le due formulazioni. Si passeranno poi in rassegna le proprietà delle forme simplettiche, per giungere infine alla \emph{formulazione simplettica} della meccanica, completamente libera da coordinate.
...
\section{Struttura simplettica canonica}
% \symplecticForm*
...
\begin{definition}
  Sia $M$ una varietà $n$-dimensionale e sia $X = T^*M$ il suo fibrato cotangente, con proiezione $\pi:X\to M$. Si dice \dfn{$1-$forma tautologica} $\lambda$ su $X$ la $1$-forma \begin{equation*}
...
\begin{definition}
  Siano $(X, \omega_X)$ e $(Y, \omega_Y)$ due varietà simplettiche. Si dice \dfn{trasformazione canonica} un diffeomorfismo $F:X\to Y$ tale che $F^*(\omega_Y) = \omega_X$. 
\end{definition}
...
\section{Formulazione simplettica}
...
\begin{definition} \label{def:hamField}
  Sia $(X, \omega)$ una varietà simplettica. Si dice \dfn{hamiltoniana} una funzione differenziabile $\mathcal{H}: X \to \R$. Si dice \dfn{campo vettoriale hamiltoniano} il campo vettoriale $X_{\mathcal{H}}$ definito su $X$ e tale che per ogni $x \in X$, per ogni $\gamma \in T_x X$
  \begin{equation} \label{eq:hamField}
    \omega(X_{\mathcal{H}}, \gamma) = \dd{\mathcal{H}}(\gamma)
  \end{equation}  
\end{definition}
